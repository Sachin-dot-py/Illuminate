...
from constants import CUR_QUARTER
...
class Scraper:
    ...
    def get_classes(self, professor_prefs=None):
        ...
        try:
            pagetext = soup.find_all("table")[2].find("td", {"align": "right"}).text.encode('ascii', 'ignore').decode(
                'ascii').strip()
                ...
                if h2:
                    ...
                    if cols:
                        ...
                        if not is_enrollable:
                            ...
                            if professor_prefs and new_class['professor'] not in professor_prefs.get(
                                    cur_dept + " " + cur_class, []):
                                continue
